...
from sklearn.model_selection import train_test_split, RandomizedSearchCV, GridSearchCV, cross_val_predict, \
    cross_val_score, LeaveOneOut, StratifiedKFold, KFold
from sklearn.feature_selection import mutual_info_classif, SelectKBest, VarianceThreshold, SelectFromModel, \
    SelectPercentile
...
from lifelines import LogNormalFitter, LogLogisticFitter, WeibullFitter, ExponentialFitter, WeibullAFTFitter, \
...
def train_and_evaluate_model(X_test, y_time_test, y_event_test,
    ...
    if estimator is None and X_train is not None and y_time_train is not None:
        ...
        if params is None:
            params = {'objective': 'survival:cox', 'eval_metric': 'cox-nloglik', 'tree_method': 'hist',
...
def generate_feature_importance_table(values, colnames, oligos_metadata, features, estimator_name,
    ...
    feature_shap_values_df['Ratio (log10)'] = np.log10(
        feature_shap_values_df[f'{group_tests[1]}'] / feature_shap_values_df[f'{group_tests[0]}'])
...
def plot_time_dependent_auc_with_table(
    ...
    # Top panel: AUC plot
    ax_auc.plot(time_points, auc_values, color=color_auc, linestyle='-', marker='', linewidth=2,
    ...
    if max_time_point not in time_points_highlight:
        time_points_highlight = list(time_points_highlight) + [
            max_time_point]  # Add it to the list if not already included
    ...
    if save_fig:
        plt.savefig(Path(figures_dir) / f'time-dependent_auc_with_table_{suffix_file}.png', dpi=300,
                    bbox_inches='tight')
...
def calculate_time_dependent_auc(y_event_train, y_time_train,
    ...
    auc_values_highlight, _ = cumulative_dynamic_auc(y_structured_train, y_structured_test, risk_scores,
...
def calculate_antigen_scores_scaled(shap_values, y_time, y_event, top_peptides, scaler=None, scaler_antigens=None,
                                    threshold=None, val_quantile=40, return_all=False):
    """
    ...
    antigen_scores_dichotomized = antigen_scores_dichotomized.rename(
        columns={antigen_scores_dichotomized.columns[0]: 'Antigen Score (Dichotomized)'})
...
def plot_kaplan_meier(df, time_column, event_column, group_column, labels,
    ...
    if not group1.empty and not group2.empty:
        ...
        ax.text(0.7, 0.05, p_value_text, transform=ax.transAxes, fontsize=10, verticalalignment='bottom',
...
def nested_cv(
    ...
    for valid_idx, fold_risk, fold_shap_df, fold_cindex, model in fold_results:
        ...
        auc_values_fold, mean_auc_values_fold = calculate_cumulative_dynamic_auc(y_time_train.loc[y_time_train.index.difference(y_valid_fold.index)] , y_valid_fold, fold_risk, time_points)
...
if __name__ == '__main__':
    ...
    model_list, train_shap_values, risk_scores_train, time_dependent_auc_train, time_dependent_auc_mean, c_index_train, validation_indices = nested_cv(X_train,
                                                                                y_time_train,
                                                                                pipeline=pipeline,
    ...
    results = {
        ...
        'time_dependent_auc_train_mean': time_dependent_auc_mean,
        'c_index_train': c_index_train,
        'validation_indices_train': validation_indices
    }

    joblib.dump(results, f'nested_cv_results_bestK_auc_{random_seed}.joblib')
