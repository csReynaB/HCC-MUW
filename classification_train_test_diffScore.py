...
from sklearn.metrics import make_scorer, roc_curve, auc,  log_loss, brier_score_loss, recall_score
...
def nested_cv_single(train_idx, valid_idx, X_train, y_train, pipeline = None,
    ...
    # Log‐loss (cross‐entropy) on the hold‐out fold
    ll = log_loss(y_valid_fold, scores_fold)

    # Brier score
    bs = brier_score_loss(y_valid_fold, scores_fold)

    # Recall at threshold t (e.g. t=0.5)
    t = 0.5
    preds_bin = (scores_fold >= t).astype(int)
    rec = recall_score(y_valid_fold, preds_bin)
    ...
    return valid_idx, scores_fold, shap_values_fold_df, best_estimator, ll, bs, rec
...
def nested_cv(
    ...
    metrics : List[Dict]
        List of dictionaries containing metrics for each outer fold.
    """
    ...
    logloss_list, brier_list, recall_list = [], [], []
    ...
    for valid_idx, fold_scores, fold_shap_df, model,  ll, bs, rec in fold_results:
    ...
    metrics = {
        ...
        'logloss_std': np.std(logloss_list, ddof=1),
    ...
    logger.info(f"Mean Brier metric across folds: {np.mean(brier_list):.4f}")
...
def train_and_validate_model(X_train: pd.DataFrame, y_train: pd.Series,
    ...
    ll = -np.mean(np.log(scores))

    # Brier score
    bs = brier_score_loss(y_test, scores)
    ...
    logger.info(f"Brier metric in testing set: {metrics_test.get('brier'):.4f}")
    ...
    return best_estimator, shap_values_df, scores, metrics_test
...
def parse_json_or_list(s):
    """
    Parse a JSON string that is either a dict or a list of dicts.
    ...
    Raises:
        argparse.ArgumentTypeError: If the input string is not a valid JSON dict or list of dicts.
    """
    obj = json.loads(s)
    ...
    if isinstance(obj, list) and all(isinstance(i, dict) for i in obj):
        # If the input is a list of dicts, return it as is
        return obj
    raise argparse.ArgumentTypeError(
        "Must be JSON dict or list of dicts, e.g. "
        "'{\"a\":1}' or '[{\"a\":1},{\"b\":2}]'"
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_test_split_data", "-sp", type=parse_json_or_list,
                        default={},
                        help=("JSON dict or list of dicts for train/test splitting. E.g. '{\"group_test\":\"Controls\"}' or "
                              "'[{\"group_test\":\"C1\"},{\"group_test\":\"C2\"}]'"))
    parser.add_argument("--train", "-t", type=parse_json_or_list,
                        default={},
                        help=("JSON dict or list of dicts for train/test splitting. E.g. '{\"group_test\":\"Controls\"}' or "
                            "'[{\"group_test\":\"C1\"},{\"group_test\":\"C2\"}]'"))
    ...
    class ValidateAction(argparse.Action):
        def __call__(self, parser, namespace, values, option_string=None):
            raw_json, out_base = values
            try:
                filters = parse_json_or_list(raw_json)
            ...
            if existing is None:
                existing = []
            # append one tuple per filter dict
            for f in filters:
                existing.append((f, out_base))
    ...
    parser.add_argument(
        ...
        action=ValidateAction, default=[],
        metavar=("FILTER_JSON", "OUT_BASENAME"),
        help=("One (or more) validation sets.  FILTER_JSON can be a single JSON object or a JSON array of objects; OUT_BASENAME is the name you’ll use in output. "
             "Examples:\n--validate '{\"treatment\":\"ICI\"}' HCC-ICI-H\n --validate '[{\"treatment\":\"ICI\"}, {\"treatment\":\"TKI\"}]' HCC-ALL\n"))
    ...
    #config_file = "/gpfs/data/fs71974/creynablanco/MLpackage/config_standard.yaml"
    ...
    if args.train_test_split_data:
        if isinstance(args.train_test_split_data, dict):
            config.filters_metadata =args.train_test_split_data
            config.combined_filters_metadata = {}
        else:
            config.filters_metadata = {}
            config.combined_filters_metadata = args.train_test_split_data
    ...
    if args.no_additional_train_test_data:
        ...
        if args.run_nested_cv:
            ...
            model_list, train_shap_values, scores_train, validation_indices, metrics_train = nested_cv(X_train,
                                                                                                       y_train,
                                                                                                       pipeline=pipeline,
        ...
        start_time = time.time()
        best_estimator, test_shap_values, scores_test, metrics_test = train_and_validate_model(X_train, y_train,
                                                                                               X_test, y_test,
                                                                                               #pipeline=pipeline,
                                                                                               param_grid=config.param_grid,
                                                                                               n_splits=outer_cv_split,
        ...
        if isinstance(args.train, dict):
            ...
            config.filters_metadata = {}
            config.combined_filters_metadata = args.train
        ...
        for filter_val, out_val in val_specs:
            ...
            if isinstance(filter_val, dict):
                ...
                config.filters_metadata = {}
                config.combined_filters_metadata = filter_val
